...
│   │   └── retriever.py        # 检索器接口
│   ├── mcp/                     # MCP 协议模块
│   │   ├── lab_server.py       # MCP Server（12 tools）
│   │   └── mcp_client.py       # MCP Client（运行时工具发现）
│   ├── spec_loader.py           # SDD 规格加载器
│   └── web/
│       └── app.py               # Streamlit 演示界面
├── config/
│   ├── product_spec.yaml        # SDD产品规格文档
│   ├── safety_incidents.yaml    # 安全事故案例库
│   └── sop_docs/                # 仪器SOP知识库（5份文档）
├── tests/
│   ├── test_system.py           # 系统集成测试
│   └── eval_benchmark.py        # Agent 评估基准
├── docs/
│   └── CS599_大作业报告.md       # 最终报告
├── requirements.txt
├── Dockerfile
