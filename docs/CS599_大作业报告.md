...
多智能体系统的核心是一个 LangGraph StateGraph 主图，包含 Supervisor 节点（意图分类 + 路由分发）、三个 Worker 子图（Scheduler/QA/Monitor，各自独立 ReAct 循环）以及一个 finalize 节点（结果透传）。AgentState 定义了全局状态结构，其中 messages 使用 Annotated 追加合并，agent_trace 通过自定义 reducer 去重保留最近 60 条追踪记录。

```python
# src/agents/graph.py — 多智能体协作主图（516 行）
...
每个 Worker Agent（Scheduler/QA/Monitor）是一个独立编译的 LangGraph StateGraph 子图。子图内部采用经典的 ReAct 模式：think 节点将 LLM 与专属工具集绑定，根据 LLM 是否返回 tool_calls 决定继续调用工具还是结束循环。最大步数限制（MAX_REACT_STEPS=8）防止 LLM 陷入无限推理。build_subagent 工厂函数根据 Agent 类型动态编译子图——Scheduler 配 7 个工具、QA 配 3 个（含 RAG）、Monitor 配 3 个。
...
系统共定义 12 个 LangChain @tool 函数，通过 Function Calling 机制由 LLM 自动选择调用。每个工具都有详细的 docstring（含参数说明和调用示例），DeepSeek/Qwen 模型根据 docstring 推断调用时机。工具按职责分为三组：Scheduler（7 个，覆盖预约全生命周期）、QA（3 个，RAG 检索 + 仪器详情）、Monitor（3 个，异常检测 + 统计 + 安全事故案例）。代码片段展示三个代表性工具的定义模式。
...
`config/product_spec.yaml` 定义了三层规格：Product Spec（痛点分析、Agent 转型方案、目标用户矩阵）、Architecture Spec（Supervisor-Worker 架构、数据模型 ER 图）、API Spec（12 个工具的完整定义，含参数、返回值、调用频次）。运行时通过 src/spec_loader.py 读取 YAML，驱动预约验证规则——修改时长限制或证书要求只需改 YAML，无需改动 Python 代码。
...
MCP 协议集成与三层防死循环机制。MCP 模块通过检测运行环境自动切换：本地无 event loop → 加载 MCP 工具，云环境有运行中 event loop → 回退内置 Function Calling。防死循环机制分三层：第一层检测"纯数字回复 + AI 刚列过仪器清单"，直接替换 prompt 推进到第二步；第二层检测"AI 问了时间 + 用户提供了时间"，代码层解析中文日期/时间/仪器名，直接调 check_availability 返回结果；第三层检测"用户说确认 + AI 刚问过确认"，从 AI 的上一条消息中正则提取日期/时段/仪器，绕过 LLM 直接调 create_booking。三层 guardrail 确保了多轮对话在 DeepSeek/Qwen 模型行为波动下仍能稳定推进。
...
`src/spec_loader.py` 是 SDD 规格驱动开发的核心执行组件。SpecLoader 以单例模式在运行时读取 product_spec.yaml，将 YAML 中的业务规则映射为 Python 验证逻辑。create_booking 工具在执行前调用 validate_booking()，从 Spec 读取时长上限进行约束校验；get_agent_tools() 从 Spec 中读取 Agent 的工具分配方案，实现"改 Spec 不改代码即可调整系统行为"。
...
gantt
    ...
    axisFormat  %Y-%m
    section Phase 1
    多智能体 + RAG + MCP + Guardrail :done, p1, 2026-06, 2026-06
    section Phase 2
...
**Phase 1：核心能力构建（2026.06 — 已完成 ✅）**

多智能体协作架构（Supervisor + 3 Worker SubGraph）、12 个 Function Calling 工具、RAG 知识库（ChromaDB + 34 文档块）、多轮对话引导、防死循环 Guardrail、多时段批量预约原子校验、MCP 协议集成（12 tools 标准化暴露）、安全事故案例库（7 条）、按用户隔离的对话持久化、SDD 规格驱动验证、自动化 Benchmark 评估（10 用例，90%+ 准确率）
