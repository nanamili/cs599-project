...
1. **增加 LLM 行为特征的专门教学。** 不同模型（DeepSeek、GPT-4、Claude）在 Function Calling 的可靠性、中文理解精度、指令遵循度上有显著差异。同一段 prompt 在不同模型上的表现可能天差地别——DeepSeek 在确认场景下倾向文字回复而非工具调用，这是模型训练偏好决定的，不是 prompt 工程师的水平问题。建议课程增加一节"模型行为分析与选型"，用对比实验的方式展示同一套 Agent 代码在不同模型上的行为差异，帮学生建立"选模型也是架构决策"的意识。同时补充 prompt engineering 的系统方法论——从指令清晰度、约束前置、反例示范、输出格式锁定四个维度做体系化讲解。

2. **强化 Guardrail 架构的课程权重。** 生产级 Agent 系统的核心不是模型有多聪明，而是代码层的兜底有多严密——确认死循环拦截、参数校验、意图分类 fallback 等。建议课程增加一个专门模块讲 Guardrail 设计模式：何时用 prompt 约束、何时用代码拦截、何时用工具返回结果校验。可以用本项目的三层拦截（意图校验→确认词拦截→工具结果验证）作为教学案例。

3. **增加 MCP 协议实践课时。** MCP 代表了 Agent 工具标准化的方向。我在项目中完整实现了 MCP Server 和 Client，但遇到了 event loop 冲突、stdio 协议调试困难等实际问题。建议课程提供一套可复现的 MCP 教学环境——从单工具注册到多工具动态发现，到跨进程通信调试——让学生真正跑通标准协议链路，而不是停留在概念层面。

4. **提供更多企业级场景的参考架构。** 课程给出的示例项目（OpenClaw、GPT Researcher）偏学术和研究向，与"企业级应用软件"的定位有距离。建议补充面向实际业务的参考架构：客服工单路由 Agent、运维告警诊断 Agent、供应链异常检测 Agent 等。这些场景天然涉及多步推理、工具调用、人机协作，能帮助学生更快找到选题方向。

5. **建议增加 Demo Day 互评环节。** 同学们的项目方向和实现路径差异很大，有的侧重对话体验、有的侧重工具链整合、有的侧重多智能体协作。互相观摩不仅能拓宽视野，还能从他人的设计决策中反思自己的方案。建议安排每位同学至少点评 2-3 个其他项目，形成书面互评记录。
