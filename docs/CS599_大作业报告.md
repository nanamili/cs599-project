...
1. **增加 LLM 行为特征的教学**：不同模型在 Function Calling 的可靠性上有显著差异。建议增加"模型选型与 prompt engineering 实战"，帮助学生理解什么时候用 prompt、什么时候用代码层兜底。
2. **提供更多业务场景案例**：现有的参考项目偏研究向，建议补充面向实际业务（客服、运维、数据分析）的 Agent 案例，帮助理解 Agent 在真实场景中的价值。
3. **建议增加 Demo Day 互评环节**：同学们的项目方向和实现差异很大，互相观摩能学到很多不同的设计思路。
