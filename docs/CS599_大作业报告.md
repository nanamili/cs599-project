...
这个项目让我完成了从"写代码实现功能"到"设计 Agent 编排业务流程"的转变。
以前拿到需求会直接想用什么库、怎么写函数，现在会先画 Agent 的状态图——
用户说什么 → Supervisor 怎么分类 → 子 Agent 有哪些工具 → 工具怎么跟数据库交互。
这个视角的转变让我理解了课程的核心——用 AI 的思维方式来设计软件架构。

SDD（规格驱动开发）也真正落实了一次。`product_spec.yaml` 不只是文档，
通过 `spec_loader.py` 实际驱动了预约验证规则。改 Spec 不改代码就能调整系统行为，
比事后补文档深刻得多。

**技术能力提升**：

LangGraph 的 StateGraph + SubGraph 架构是这次最大的技术收获。
从 v1 的单 Agent ReAct 到 v3 的三个独立编译子图，每一步重构都踩了坑——
子图不带 MemorySaver 导致多轮对话失忆、DeepSeek 偏好文字回复而非 Function Calling、
消息只传第一条 HumanMessage 导致上下文丢失。
这些问题逼迫我去理解 State 的 Annotated 合并机制、checkpointer 的作用域、
以及什么时候该用代码层拦截绕过 LLM。

RAG 的完整链路也跑通了——从 Markdown 加载、Sentence-Transformer 嵌入、
ChromaDB 索引、到 Agent 在回复中标注来源。MCP 协议的集成让我理解了
工具标准化暴露和运行时发现的架构价值。

**对 Agentic AI 的理解**：

Agent 不是 ChatBot 套个 prompt。核心在于"自主决策 + 工具使用 + 多步推理"。
好设计需要三个要素精密配合：State（消息历史怎么传、用户上下文怎么注入）、
Tool（描述要具体到参数格式和调用时机）、Prompt（边界情况和拒绝条件）。

另外深刻体会到当前 LLM 还不够可靠——即使 Prompt 写得很清楚，
模型在某些场景下仍会无视指令。生产级 Agent 系统需要代码层 guardrail 来兜底，
不能完全信任 LLM 的输出。
