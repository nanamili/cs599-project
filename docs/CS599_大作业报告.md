...
这门课最核心的价值，是让我完成了从"面向功能编程"到"面向状态+工具+决策的 Agent 编排"的思维跃迁。

**从确定性到概率性**。传统软件开发的核心是控制——条件分支、循环、类型系统，
一切都是确定性的。Agent 系统引入了一个根本变化：核心推理单元（LLM）的输出是不确定的。
同样的 prompt、同样的上下文，模型可能给出不同的回复、选择不同的工具。这不再是 bug，
而是系统设计时需要接受的常态。应对之道不是消除不确定性，而是用确定性的代码层
（Guardrail）去容错和兜底——校准意图分类、拦截死循环、校验工具参数。

**从写代码到画状态图**。过去拿到需求先想"用什么库、写什么函数"，
现在先画 StateGraph——消息怎么流转、用户上下文怎么注入、子图之间怎么通信。
State 设计成了最重要的架构决策：Annotated 的 reducer 怎么写、
checkpointer 的作用域怎么定、子图是否独立编译——这些决定了系统的行为边界。

**SDD：规格先行**。以前写文档是为了交差，这次 `product_spec.yaml` 真正驱动了系统行为——
预约验证规则从 Spec 读取，改 YAML 不改代码就能调整约束。先 Spec 后 Code 不只是方法论，
是实际的工程效率提升。

**AI 负责推理，代码负责兜底**。经过无数次"模型无视 prompt 指令"的折磨，
我形成了这个核心认知：LLM 擅长意图识别和自然语言生成，但关键操作不能依赖它的自律。
确认预约不能指望模型主动调 create_booking——代码层拦截确认词、直接解析参数、
绕过 LLM 执行，这才是生产级的做法。
